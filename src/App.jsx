...
export default function App() {
  ...
  useEffect(() => {
    socket.connect();
    socket.on('update_room', (data) => setGameState(data));
  ...
  const renderCard = (card, idx) => {
    if (card === 'HIDDEN') {
      return (
        <div key={idx} className="w-16 h-24 bg-blue-800 border-2 border-white text-white flex items-center justify-center rounded shadow-lg font-bold text-xs rotate-y-180">
          BJ
        </div>
      );
    }
    ...
    return (
      <div key={idx} className="w-16 h-24 bg-white flex items-center justify-center rounded shadow-lg font-bold text-xl border border-gray-300">
        <span className={isRed ? 'text-red-600' : 'text-black'}>{card}</span>
      </div>
    );
  };
  ...
  if (!gameState) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen gap-4">
        ...
        <input className="px-4 py-2 text-black rounded" placeholder="방 번호 (예: 1)" value={roomId} onChange={e => setRoomId(e.target.value)} />
        <input className="px-4 py-2 text-black rounded" placeholder="닉네임" value={playerName} onChange={e => setPlayerName(e.target.value)} />
    ...
    <div className="flex flex-col max-w-5xl p-4 mx-auto min-h-screen">
      <header className="flex justify-between items-center mb-6 border-b border-gray-700 pb-4">
        <div>
          ...
          <p className="text-sm text-gray-300">잔여 카드: {gameState.remainingCards}장 | 내 칩: 🪙 {me?.chips}</p>
        </div>
        ...
        <div className="md:col-span-2 space-y-6 flex flex-col">
          ...
          <div className="p-6 bg-slate-800/80 rounded-xl border border-gray-600 flex flex-col items-center justify-center min-h-[160px]">
            ...
            <div className="flex gap-2 justify-center mb-2">
              {gameState.dealer.hand.map((card, i) => renderCard(card, i))}
            </div>
          ...
          <div className="flex-1 space-y-3 overflow-y-auto pr-2">
            {gameState.players.map((player, idx) => {
              ...
              return (
                <div key={player.id} className={`p-4 rounded-xl border flex justify-between items-center transition-all ${player.id === socket.id ? 'border-green-500 bg-green-900/30' : 'border-gray-700 bg-slate-800'} ${active ? 'ring-2 ring-yellow-400 transform scale-[1.02]' : ''}`}>
                  <div className="flex flex-col gap-1 w-1/3">
                    <h3 className="font-semibold text-lg">{player.name} {player.id === socket.id && '(나)'}</h3>
                    <span className="text-sm text-gray-300">🪙 {player.chips} | 💰 Bet: {player.bet}</span>
                    <span className="text-xs px-2 py-1 bg-slate-900 rounded w-max uppercase">{player.state}</span>
          ...
          <div className="bg-slate-800 p-4 rounded-xl border border-gray-700 min-h-[100px] flex items-center justify-center">
            ...
            {gameState.status === 'playing' && !isMyTurn && <p className="text-gray-400">다른 플레이어의 턴을 기다리는 중...</p>}

            {/* 3. 라운드 종료 */}
            {gameState.status === 'finished' && me?.state !== 'ready_next' && (
        ...
        <div className="md:col-span-1 bg-black/60 border border-gray-700 rounded-xl flex flex-col h-[700px]">
          ...
          <div className="flex-1 p-3 overflow-y-auto text-sm space-y-2 font-mono">
            {gameState.logs.map((log, idx) => (
              <div key={idx} className={log.includes('BUST') || log.includes('결과') ? 'text-yellow-400 font-bold' : log.includes('시스템') ? 'text-gray-500' : 'text-gray-300'}>
                {log}
              </div>
            ))}
            <div ref={logsEndRef} />
